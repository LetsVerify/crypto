mod tests {
    ...
    fn test_pok_interactive() {
        ...
        // Prover Step 1
        let (commitment, state) = pok_commit(&params, &messages, &signature, &disclosed_indices, &mut rng).unwrap();
        ...
        // Verifier Step 2
        let is_valid = pok_verify(&params, &pk, &disclosed_msgs, &commitment, &challenge, &response);
    ...
    fn test_pok_non_interactive() {
        ...
        // NIZK Prove
        let proof = nizk_prove(&params, &pk, &messages, &signature, &disclosed_indices, &mut rng).unwrap();
    ...
    fn test_pok_non_interactive_prefix() {
        ...
        // NIZK Prove
        let proof = nizk_prove_prefix(&params, &pk, &messages, &signature, disclosed_count, &mut rng).unwrap();
